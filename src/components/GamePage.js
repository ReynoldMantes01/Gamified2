...
const GamePage = ({
  ...
  const [definition, setDefinition] = useState({
    text: "Definition shows here when you enter right",
    source: "",
  });
  ...
  const [currentWorldBackground, setCurrentWorldBackground] =
    useState("Bio_World.gif");
...
>>>>>>> 6cd9df86ee7f28041320f87706594accf47a390f
        const db = getDatabase();
        const userRef = ref(db, `users/${auth.currentUser.uid}/health`);
        try {
          const snapshot = await get(userRef);
  ...
  useEffect(() => {
    ...
    setEnemyProgression(progression);
    console.log(
      "Enemy progression loaded:",
      progression.map((e) => e.id)
    );
  }, []);
  ...
  const isValidWord =
    ...
    scienceTerm[selectedLetters.join("").toUpperCase()];
  const [isValidWordState, setIsValidWord] = useState(false);
  ...
  const handleGameOver = async (isVictory) => {
    ...
    if (isVictory) {
      ...
      console.log(
        "Victory! Updating user progress for enemy:",
        currentEnemy?.id
      );
...
>>>>>>> 6cd9df86ee7f28041320f87706594accf47a390f
      ...
      //Check if this was the last enemy (game fully beaten)
      if (
        enemyProgression.length === 0 ||
        currentEnemyIndex >= enemyProgression.length - 1
      ) {
        console.log("Game fully cleared! Stopping Timer.");
...
>>>>>>> 6cd9df86ee7f28041320f87706594accf47a390f
  ...
  useEffect(() => {
    if (enemyHearts <= 0 && currentEnemy) {
      console.log(
        "Enemy hearts reached zero, triggering defeat:",
        currentEnemy.id
      );
      handleGameOver(true);
    }
  ...
  const addRandomEffect = () => {
    console.log(
      "Checking for effect addition, word length:",
    ...
    if (selectedLetters.length >= 2) {
      ...
      // Get available spots (those with letters but no effects)
      const availableSpots = gridLetters
  ...
  const handleAttack = () => {
    ...
    if (scienceTerm[word]) {
      ...
      // Show attack feedback with status effects
      let feedbackText = `You dealt ${Math.ceil(
        totalDamage
      )} damage with "${word}"!`;
      if (statusEffectApplied) {
        feedbackText += " Status effect applied!";
      ...
      if (updatedHearts <= 0) {
        ...
        console.log(
          "Enemy damaged but not defeated. Hearts remaining:",
          updatedHearts
        );
      }
    } else {
  ...
  const handleNextLevel = () => {
    ...
    // Find the current enemy index in the progression
    const currentEnemyIndex = enemyProgression.findIndex(
      (e) => e.id === currentEnemy?.id
    );
    console.log("Current enemy index:", currentEnemyIndex);
    ...
    if (
      ...
      if (currentMapId !== nextMapId) {
        ...
        // Return to map selection screen for map transition
        console.log(
          "Forcing reload of user progress before returning to main menu"
        );
        reload();
        setTimeout(() => {
          onMainMenu();
  ...
  const tutorialSteps = [
    {
      message:
        "Welcome to Science Quest! I'm Professor Nova, and I'll be your guide in this adventure of scientific discovery!",
      highlight: null,
    },
    {
      message:
        "Your mission is to defeat enemies by forming scientific words(BIOLOGY, CHEMISTRY, PHYSICS). The longer and more complex the word, the more damage you'll deal!",
      highlight: "word-box",
    },
    {
      message:
        "Click/Type on letters to form words. Each correct scientific term will launch an attack!",
      highlight: "letter-grid",
    },
    {
      message:
        "Use HINTS when you're stuck, but use them wisely - you only have 3 per level!",
      highlight: "hint-box ",
    },
    {
      message: "If you can't find any words, use SCRAMBLE to get new letters.",
  ...
  const handleClickOutside = useCallback(
    (event) => {
      if (
        ...
        !event.target.closest(".slidebar-icon")
      ) {
        setSlidebarOpen(false);
      }
    },
  ...
  const handleSettingsSave = (
    newMusicVolume,
    newSoundEffectsVolume,
    newBackgroundVolume
  ) => {
    console.log(
      "GamePage: Saving settings with volumes:",
      newMusicVolume,
      newSoundEffectsVolume,
      newBackgroundVolume
    );
  ...
  const updateUserProgress = async (defeatedEnemy) => {
    ...
    // Normalize the defeated enemy name for storage
    const defeatedEnemyName = defeatedEnemy.name
      .toLowerCase()
      .replace(" ", "_");
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (
          currentEnemyIndex !== -1 &&
          currentEnemyIndex < allEnemies.length - 1
        ) {
  ...
  const handleVictory = () => {
    ...
    console.log(
      "Updating user progress with enemy including mapId:",
      enemyWithMapId
    );
  ...
  const isEnemyLocked = (enemyId) => {
    ...
    return !userProgress.unlockedEnemies.some((unlockedId) => {
      const unlockedIndex = enemyProgression.findIndex(
        (e) => e.id === unlockedId
      );
  ...
  const resetGame = () => {
    ...
    if (currentEnemy) {
      ...
      // Set dialog for the new enemy
      const enemyDialog =
        currentEnemy.dialog || `${currentEnemy.name} appears!`;
      setDialogSequence([enemyDialog]);
      setCurrentDialogIndex(0);
      setDialogMessage(enemyDialog);
      setDialogVisible(true);
  ...
  const handleKeyPress = useCallback(
    (event) => {
      ...
      if (/^[A-Z]$/.test(key)) {
        ...
        const letterIndex = gridLetters.findIndex(
          (letter, index) => letter === key && !emptyIndices.includes(index)
        );
      ...
      // Handle backspace for letter removal
        ...
        if (selectedLetterIndex >= 0) {
          handleSelectedLetterClick(
            selectedLetters[selectedLetterIndex],
            selectedLetterIndex
          );
          setSelectedLetterIndex(
            Math.min(selectedLetterIndex, selectedLetters.length - 2)
          );
        } else {
        ...
        >
          <div
            className={`character 
                                    ...
                                    ${
                                      isCharacterAttacking
                                        ? "character-responsive translate-x-24 md:translate-x-32 lg:translate-x-40"
                                        : ""
                                    }
                                    ${
                                      isCharacterIdle
                ...
                className={` 
                    ...
                    ${
                      index === selectedLetterIndex
                        ? "border-teal-500 bg-blue-100 scale-105"
                        : "border-gray-700"
                    }
                    ${
                      highlightedIndices.includes(index)
                        ? "bg-green-300 scale-105 shadow-md"
      ...
      >
        ...
        >
          <div className="text-gray-800 leading-relaxed text-sm md:text-base">
            {definition.text ||
              "Discover a scientific term to reveal its definition."}
          </div>
          ...
          <div className="action-buttons mt-2 md:mt-8 flex flex-col space-y-1 md:space-y-2">
            ...
            <div className="hint-box bg-blue-50 border-2 border-gray-700 py-1.5 md:py-2 px-2 md:px-3 text-center rounded-lg shadow-inner">
              <p className="text-xs md:text-sm truncate font-medium">
                {hint || `Lab assistance: ${hintsRemaining}`}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                className={`scramble-button bg-blue-50 text-gray-800 border-2 border-gray-700 py-1.5 md:py-2 text-xs md:text-sm font-bold rounded-lg transition-all ${
                  scrambleCooldown
                    ? "opacity-50 cursor-not-allowed"
                    : "hover:bg-blue-100 hover:shadow-md transform hover:-translate-y-0.5"
                }`}
            ...
            {showTutorial ? (
              ...
              <>
                ...
                <button
                  ...
                  onClick={handleNextDialog}
                >
                  {currentDialogIndex < dialogSequence.length - 1
                    ? "Continue Scan"
              ...
              {tutorialSteps.map((_, index) => (
                <div
                  ...
                  className={`h-2 w-2 rounded-full ${
                    showTutorial && index === tutorialStep
                      ? "bg-teal-400"
                      : showTutorial && index < tutorialStep
                      ? "bg-teal-700"
                      : "bg-gray-700"
